...
setLoadingAI(true);
const prompt=`You are a personal trainer. Suggest 6 alternative exercises to swap for "${exercise.name}" (muscle: ${exercise.muscle||"unknown"}). Requirements: joint-friendly for a 49-year-old, similar muscle group, gym equipment available. Return ONLY a JSON array of objects: [{"name":"Exercise Name","sets":"3","reps":"10-12","note":"brief reason","muscle":"${exercise.muscle||""}"}] No markdown, no explanation, just the array.`;
try{
...
alert(raw?`Raw storage: ${raw.slice(0,500)}...`:“Nothing in forge_sessions”);
}}>
Inspect Raw Storage
</Btn>
</div>}
...
1. ONE form or progression tip for the current exercise
   Plain text, no markdown, be concise and direct.` :`You are a personal trainer analyzing a workout day for a 49-year-old male focused on hypertrophy. Sensitive knees.
   Program: Plan B Antagonist Split, started May 2 2026, currently week ${programWeek()}.
...
1. Any muscle gaps or imbalances
1. One concrete optimization suggestion
