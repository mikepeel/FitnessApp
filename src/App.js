  ...
  return <div>
    ...
    <div style={{padding:"14px 18px"}}>
      ...
      {orderedDays.map((day,i)=>{
        ...
        const dayDOW=DOW.indexOf(day.name); // DOW uses full names: Sunday, Monday...
        const diffDays=dayDOW>=0?dayDOW-todayDOW:0;
        const dayDate=new Date(todayDate);
        dayDate.setDate(todayDate.getDate()+diffDays);
        const dayDateStr=dayDate.toISOString().split("T")[0];
        const doneSess=dayDOW>=0&&sessions.some(s=>s.completedAt?.startsWith(dayDateStr));
